...
```json
{
  "sessionData": {
    ...
    "data": [
      ...
      { "columnName": "Tutorial/Clicks/Button B", "value": 0 }
    ]
  }
}
```
...
`metaData` is a **lean routing envelope** — only the fields needed to route the document to the correct Firestore path and schema. All timing and device data lives exclusively in the `Session/` group of the `data` array.
...
| `schemaVersion` | string | Version of the `SessionSchema` ScriptableObject at the time the session was sent |
| `sessionId` | string | UUID v4. Generated by the SDK at session start |

Do not add `platform`, `startedAt`, `endedAt`, or any other analytics values to `metaData` — those belong in the `data` array under the `Session/` namespace. Do not add more `metaData` fields without a corresponding update to this doc and to the `AnalyticsSession` runtime class.
